...
export function Admin() {
  ...
  const fetchTournaments = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://158.160.49.96:5085/api/tournaments', {
        headers: {
  ...
  const startNewTournament = async () => {
    try {
      ...
      const response = await fetch('http://158.160.49.96:5085/api/tournaments/tournament/create', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteTournament = async (tournamentId: string, tournamentName: string) => {
    if (window.confirm(`Вы уверены, что хотите удалить турнир "${tournamentName}"?`)) {
      try {
        const response = await fetch(`http://158.160.49.96:5085/api/tournaments/tournament/delete/${tournamentId}`, {
          method: 'DELETE',
          headers: {
            'accept': 'text/plain',
            "Access-Control-Allow-Origin": "*"
